...
// Standalone routes do not mount TinodeWeb, so establish the same root theme
// attributes before their first paint.
export function applyThemeAttributes(value, target = globalThis.document) {
  const theme = normalizeTheme(value);
  const root = target?.documentElement;
  if (root) {
    root.dataset.theme = isLiquidTheme(theme) ? 'liquid' : theme;
    if (theme === 'liquid-green') {
      root.dataset.liquidVariant = 'green';
    } else {
      delete root.dataset.liquidVariant;
    }
  }
