...
describe('MessagesView composer draft isolation', () => {
  ...
  it('allows up to 100 messages for links while keeping share images capped at 50 items', async () => {
    api.getMessages.mockResolvedValueOnce({
    ...
    expect(toggles).toHaveLength(101);
    ...
    expect(toolbar?.textContent).toContain('已选 100 条');

    await act(async () => toggles[100].click());
    expect(toolbar?.textContent).toContain('已选 100 条');
    expect(toolbar?.textContent).toContain('只读链接最多选择 100 条消息。');
    ...
    await act(async () => {
      Simulate.submit(container.querySelector('.cc-conversation-link-share-review form'));
      await flushPromises();
    });
    const payload = api.createConversationShare.mock.calls[0][0];
    expect(payload).toMatchObject({
      topicId: 'p2p_1_2',
      title: '会话片段',
      expiresIn: 604800,
    });
    expect([...payload.messageIds].sort((left, right) => left - right)).toEqual(
      Array.from({ length: 100 }, (_, index) => index + 1),
    );
  });

  it('keeps a text message with internal blocks selectable and sends only its source ID', async () => {
    api.getMessages.mockResolvedValueOnce({
      messages: [{
        id: 190,
        seq_id: 190,
        ...
        type: 'text',
        content_blocks: [
          { type: 'text', text: '这是可以分享的最终结论。' },
          { type: 'thinking', thinking: '不能出现在访客页面的推理。' },
        ],
      }],
    });
    await mountTopic(root, 'p2p_1_2');
    ...
    await act(async () => {
      container.querySelector('.mock-chat-message[data-message-id="190"] .mock-create-conversation-share').click();
      await flushPromises();
    });
  ...
  it('does not consume an older repeated prompt as the pending send', async () => {
    const historyResult = deferred();
    const sendResult = deferred();
    mockTutorialAgentPeer();
    api.getMessages.mockReturnValueOnce(historyResult.promise);
  ...
  it('keeps a live Agent reply after an unresolved user send during history merge', async () => {
    const historyResult = deferred();
    const sendResult = deferred();
    mockTutorialAgentPeer();
    api.getMessages.mockReturnValueOnce(historyResult.promise);
  ...
  it('does not send structured mentions after typing against the picker token boundary', async () => {
    ...
    expect(api.sendMessage).toHaveBeenCalledWith('grp_80', '@Wanyux 请处理', undefined, undefined, expect.any(String));
  });
  ...
  it('drops structured mention provenance after the picker token is removed', async () => {
    ...
    expect(api.sendMessage).toHaveBeenCalledWith('grp_80', '@Wanyu 请处理', undefined, undefined, expect.any(String));
  });
