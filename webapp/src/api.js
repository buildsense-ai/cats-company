...
const MEDIA_BASE = (() => {
  const explicit = String(import.meta.env.VITE_MEDIA_BASE || '').replace(/\/+$/, '');
  if (explicit) return explicit;
  if (import.meta.env.DEV || typeof window === 'undefined') return '';
  return mediaBaseForHostname(window.location.hostname);
})();
...
export const api = {
  ...
  renameCloudArtifactTag: (agentUid, tag, nextTag) =>
    request('PUT', `/api/agents/${encodeURIComponent(agentUid)}/artifacts/tags/${encodeURIComponent(tag)}`, { tag: nextTag }),
};
