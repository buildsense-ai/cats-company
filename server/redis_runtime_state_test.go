...
func TestRedisRuntimeClearStaleDeviceRouteDoesNotRemoveReplacement(t *testing.T) {
	...
	oldRoute := runtimeRoute{NodeID: "node-a", ConnectionID: "old-conn", ExpiresAt: time.Now().Add(time.Minute)}
	newRoute := runtimeRoute{NodeID: "node-b", ConnectionID: "new-conn", ExpiresAt: time.Now().Add(time.Minute)}

	state.bindUserDeviceRoute(7, device, oldRoute)
	state.bindUserDeviceRoute(7, device, newRoute)
	state.clearUserDeviceRoute(7, device.DeviceID, oldRoute)

	got, ok := state.userDeviceRoute(7, device.DeviceID, time.Now())
...
func TestRedisRuntimeRouteRequiresLiveNodeHeartbeat(t *testing.T) {
	...
	route := runtimeRoute{NodeID: "node-a", ConnectionID: "conn-a", ExpiresAt: time.Now().Add(time.Minute)}
	state.bindRuntimeRoute(route)
	if !state.routeConnected(route) {
