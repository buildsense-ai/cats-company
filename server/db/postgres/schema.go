...
func (a *Adapter) CreateSchema() error {
	statements := []string{
		...
		// Per-plan policy for whether the startup reconcile may add new relay
		// models to a plan. It must exist before the reconcile runs, and it is
		// not a model list, so it does not belong in the ordered block below.
...
const createAgentArtifactTagsTable = `
...
// Historical (2026-07): two-model paid packages became all-model packages,
// keeping the shared Relay pool size unchanged. Superseded by the relay-catalog
// reconcile; kept as an ordered placeholder.
const migrateCommercialPaidPlansAllModels = `
-- Historical migration, now a no-op: Two-model paid packages became all-model packages.
--
...
// Historical (2026-08): GLM-5.3-Flash joined every public plan without
// increasing the paid shared pool. Superseded by the relay-catalog reconcile;
// kept as an ordered placeholder.
const migrateCommercialPlansGLM53Flash = `
-- Historical migration, now a no-op: GLM-5.3-Flash joined the paid plans.
--
-- The plan model sets used to be written here as literal JSON, which
-- meant every relay model launch needed a control-plane change and each
-- startup re-applied the same list, silently reverting a hand-edited
