...
func (a *Adapter) ListMutedConversationTopics(ctx context.Context, userID int64, topicIDs []string) (map[string]bool, error) {
	...
	rows, err := a.db.QueryContext(ctx,
		fmt.Sprintf(`SELECT topic_id FROM conversation_notification_mutes WHERE user_id = $1 AND topic_id IN (%s)`, inPlaceholders(2, len(topicIDs))),
		args...,
	)
	if err != nil {
...
func (a *Adapter) SetConversationNotificationsMuted(ctx context.Context, userID int64, topicID string, muted bool) error {
	if muted {
		if _, err := a.db.ExecContext(ctx,
			`INSERT INTO conversation_notification_mutes (user_id, topic_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
			userID, topicID,
		); err != nil {
	...
	if _, err := a.db.ExecContext(ctx,
		`DELETE FROM conversation_notification_mutes WHERE user_id = $1 AND topic_id = $2`,
		userID, topicID,
	); err != nil {
...
func (a *Adapter) IsConversationNotificationsMuted(ctx context.Context, userID int64, topicID string) (bool, error) {
	...
	if err := a.db.QueryRowContext(ctx,
		`SELECT EXISTS(SELECT 1 FROM conversation_notification_mutes WHERE user_id = $1 AND topic_id = $2)`,
		userID, topicID,
	).Scan(&muted); err != nil {
