	...
	rows, err := a.db.Query(
		`SELECT u.id, u.username, u.display_name, COALESCE(u.avatar_url, ''),
		  ...
		  WHERE u.account_type = 'bot'
		    AND u.state = 0
		    AND COALESCE(b.enabled, true) = true
		    AND b.owner_id = $1
	...
	rows, err := a.db.Query(
		`SELECT u.id, u.username, u.display_name, COALESCE(u.avatar_url, ''),
		  ...
		  WHERE u.account_type = 'bot'
		    ...
		    AND COALESCE(b.owner_id, 0) <> $1
		    AND NOT EXISTS (
		      SELECT 1 FROM agent_access aa
		       WHERE aa.agent_uid = u.id AND aa.user_uid = $1
...
func (a *Adapter) GetAgentAccess(agentUID, userUID int64) (*types.AgentAccess, error) {
	rows, err := a.db.Query(
		`SELECT aa.id, aa.agent_uid, aa.user_uid,
		   ...
		   JOIN users u ON u.id = aa.user_uid
		   LEFT JOIN users inviter ON inviter.id = aa.invited_by
		  WHERE aa.agent_uid = $1 AND aa.user_uid = $2
		  LIMIT 1`,
		agentUID, userUID,
	)
	if err != nil {
...
func (a *Adapter) UpsertAgentAccess(agentUID, userUID, invitedBy int64, permission, status, source string) (*types.AgentAccess, error) {
	...
	err := a.db.QueryRow(
		`INSERT INTO agent_access (agent_uid, user_uid, permission, status, source, invited_by, accepted_at)
		 VALUES ($1, $2, $3, $4, $5, $6, CASE WHEN $4 = 'active' THEN CURRENT_TIMESTAMP ELSE NULL END)
		 ON CONFLICT (agent_uid, user_uid)
		 DO UPDATE SET
		   permission = EXCLUDED.permission,
		   status = EXCLUDED.status,
		   source = EXCLUDED.source,
		   invited_by = EXCLUDED.invited_by,
		   accepted_at = CASE WHEN EXCLUDED.status = 'active' THEN COALESCE(agent_access.accepted_at, CURRENT_TIMESTAMP) ELSE NULL END
		 RETURNING id`,
		agentUID, userUID, permission, status, source, invitedBy,
	).Scan(&id)
	if err != nil {
	...
	err := a.db.QueryRow(
		`UPDATE agent_access
		    SET permission = $1,
		        status = $2,
		        accepted_at = CASE
		          WHEN $2 = 'active' THEN COALESCE(accepted_at, CURRENT_TIMESTAMP)
		          WHEN $2 = 'pending_accept' THEN NULL
		          ELSE accepted_at
		        END
		  WHERE id = $3 AND agent_uid = $4
		  RETURNING id`,
		permission, status, accessID, agentUID,
	).Scan(&id)
	if err == sql.ErrNoRows {
		return nil, nil
	}
...
func (a *Adapter) RevokeAgentAccess(accessID, agentUID int64) error {
	_, err := a.db.Exec(
		`UPDATE agent_access SET status = 'revoked' WHERE id = $1 AND agent_uid = $2`,
		accessID, agentUID,
	)
	return err
}
...
func (a *Adapter) AcceptAgentInvite(agentUID, userUID int64) (*types.AgentAccess, error) {
	...
	err := a.db.QueryRow(
		`UPDATE agent_access
		  ...
		  WHERE agent_uid = $1 AND user_uid = $2 AND status = 'pending_accept'
		  RETURNING id`,
		agentUID, userUID,
	).Scan(&id)
	if err == sql.ErrNoRows {
