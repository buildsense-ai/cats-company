...
BEGIN
    FOR package IN
        ...
        GROUP BY g.uid, g.plan_id, g.grant_type, g.source_ref,
                 g.effective_at, g.expires_at, g.reset_duration
        HAVING COUNT(*) FILTER (WHERE g.model IN ('gpt-5.6-sol', 'gpt-5.6-luna')) > 0
    ...
    LOOP
        ...
        LOOP
            ...
            VALUES (package.uid, grant_row.model, -grant_row.amount_cny, 'revoke',
                    'public_models_v2', grant_row.id, 'Public plan model access update');
        ...
        FOREACH public_model IN ARRAY ARRAY[
            ...
            model_amount := CASE WHEN public_model = 'gpt-5.6-terra'
                THEN package.total - ROUND(package.total / 5, 6) * 4
                ELSE ROUND(package.total / 5, 6) END;
            INSERT INTO commercial_quota_grants(
                ...
                package.expires_at, package.source_ref, 'Public plan model access update', package.operator_uid
            ) RETURNING id INTO new_grant_id;
            INSERT INTO commercial_quota_ledger(uid, model, amount_cny, entry_type, source_type, source_id, note)
            VALUES (package.uid, public_model, model_amount, 'grant',
                    'public_models_v2', new_grant_id, 'Public plan model access update');
        END LOOP;
    END LOOP;
END $$;
`
