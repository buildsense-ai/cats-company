...
// DeepSeek V4 retirement (2026-09-23): remove the retired public model from
// every commercial package. Paid plans keep their advertised totals (11000 /
// 33000) by splitting the same allowance over the five remaining chat models,
// and the Free pool drops the model entirely. Exact prior-state predicates
// keep the migration idempotent and leave custom or legacy quota untouched.
const migrateCommercialPlansRetireV4Flash = `-- Retire DeepSeek V4 Flash from every public commercial package. The paid
...
// rollbackCommercialPlansRetireV4Flash restores the six-model paid packages
// and the ten-model Free package (with the V4 grant) for rollback and drift
// testing.
const rollbackCommercialPlansRetireV4Flash = `-- Roll back the DeepSeek V4 retirement: restore the six-model paid packages
