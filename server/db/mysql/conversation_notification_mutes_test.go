...
func TestConversationNotificationMutesStoreAndReadByUserAndTopic(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO conversation_notification_mutes (user_id, topic_id) VALUES (?, ?)
			 ON DUPLICATE KEY UPDATE user_id = VALUES(user_id)`)).
		WithArgs(uid, mutedID).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT topic_id FROM conversation_notification_mutes WHERE user_id = ? AND topic_id IN (?,?)`)).
		WithArgs(uid, mutedID, unmutedID).
		WillReturnRows(sqlmock.NewRows([]string{"topic_id"}).AddRow(mutedID))
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT EXISTS(SELECT 1 FROM conversation_notification_mutes WHERE user_id = ? AND topic_id = ?)`)).
		WithArgs(uid, mutedID).
		WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(true))
	mock.ExpectExec(regexp.QuoteMeta(`DELETE FROM conversation_notification_mutes WHERE user_id = ? AND topic_id = ?`)).
		WithArgs(uid, mutedID).
		WillReturnResult(sqlmock.NewResult(0, 1))
