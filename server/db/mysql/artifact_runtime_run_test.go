...
func TestMySQLCreateArtifactRuntimeRunEnforcesDatabaseAdmissionLimits(t *testing.T) {
	tests := []struct {
		name       string
		active     int
		recent     int
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			policy := mysqlRuntimeRunCreatePolicy()
			mock.ExpectBegin()
			...
			mock.ExpectQuery(`SELECT COUNT\(\*\) FROM artifact_runtime_runs\s+WHERE actor_uid = \? AND status`).
				WithArgs(int64(7), now).
				WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(test.active))
			if test.expectRate {
				mock.ExpectQuery(`SELECT COUNT\(\*\) FROM artifact_runtime_runs\s+WHERE actor_uid = \? AND created_at`).
			...
			_, err = (&Adapter{db: db}).CreateArtifactRuntimeRun(
				context.Background(), mysqlRuntimeRunCandidate(now), policy,
			)
...
func TestMySQLCreateArtifactRuntimeRunPrunesRetainedRunAndEvents(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
	...
	mock.ExpectQuery(`SELECT .* FROM artifact_runtime_runs WHERE task_id = \? AND actor_uid = \?`).
		WithArgs("task-1", int64(7)).
		WillReturnRows(mysqlRuntimeRunTestRowWithAccess("submitted", "", false, false, nil, nil, now.Add(time.Hour), now))
	mock.ExpectCommit()
...
func TestMySQLCreateArtifactRuntimeRunPrunesCapacityAcrossBatches(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
...
func TestMySQLCreateArtifactRuntimeRunRejectsFullPersistentStoreWithoutTerminalVictim(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
...
func TestMySQLObserveArtifactRuntimeExecutorCompletesMetadataAfterBusinessRunFinished(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
	...
	mock.ExpectQuery(`SELECT .* FROM artifact_runtime_runs WHERE task_ref_hash = \? FOR UPDATE`).
		WithArgs("ref-hash").
		WillReturnRows(mysqlRuntimeRunTestRowWithAccess("completed", "running", false, true, nil, nil, now.Add(time.Hour), now))
	mock.ExpectExec(`UPDATE artifact_runtime_runs SET\s+executor_run_id = \?, executor_state = \?, executor_finished_at = \?, updated_at = \?`).
...
func TestMySQLArtifactRuntimeRunMutationsRevalidateDeliveryAndExpiryUnderLock(t *testing.T) {
	tests := []struct {
		name      string
		complete  bool
		delivered bool
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`SELECT .* FROM artifact_runtime_runs WHERE task_ref_hash = \? FOR UPDATE`).
				WithArgs("ref-hash").
				WillReturnRows(mysqlRuntimeRunTestRowWithAccess("running", "running", false, test.delivered, nil, nil, expiresAt, now))
			mock.ExpectRollback()
...
func TestMySQLReserveArtifactRuntimeDeliveryRecoversExpiredLeaseWithoutDuplicateDelivery(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
	...
	mock.ExpectQuery(`SELECT .* FROM artifact_runtime_runs WHERE task_ref_hash = \? FOR UPDATE`).
		WithArgs("ref-hash").
		WillReturnRows(mysqlRuntimeRunTestRowWithAccess("submitted", "", true, false, &staleClaim, nil, now.Add(time.Hour), now))
	mock.ExpectExec(`UPDATE artifact_runtime_runs\s+SET delivery_claimed = 1`).
		WithArgs("message-1", sqlmock.AnyArg(), sqlmock.AnyArg(), "task-1").
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
	adapter := &Adapter{db: db}
	...
	mock.ExpectExec(`UPDATE artifact_runtime_runs\s+SET delivery_claimed = 0, delivery_claimed_at = NULL, delivered = 1`).
	...
	mock.ExpectQuery(`SELECT .* FROM artifact_runtime_runs WHERE task_ref_hash = \? FOR UPDATE`).
		WithArgs("ref-hash").
		WillReturnRows(mysqlRuntimeRunTestRowWithAccess("submitted", "", false, true, nil, nil, now.Add(time.Hour), now))
	mock.ExpectRollback()
...
func TestMySQLReserveArtifactRuntimeDeliveryKeepsLiveLeaseExclusive(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
	...
	mock.ExpectQuery(`SELECT .* FROM artifact_runtime_runs WHERE task_ref_hash = \? FOR UPDATE`).
		WithArgs("ref-hash").
		WillReturnRows(mysqlRuntimeRunTestRowWithAccess("submitted", "", true, false, &now, nil, now.Add(time.Hour), now))
	mock.ExpectRollback()
...
func mysqlRuntimeRunCreatePolicy() store.ArtifactRuntimeRunCreatePolicy {
...
func mysqlRuntimeRunCandidate(now time.Time) *store.ArtifactRuntimeRun {
...
func mysqlRuntimeRunTestRow(status, executorState string, now time.Time) *sqlmock.Rows {
	finishedAt := now
	return mysqlRuntimeRunTestRowWithAccess(status, executorState, false, true, nil, &finishedAt, now.Add(time.Hour), now)
}

func mysqlRuntimeRunTestRowWithAccess(
