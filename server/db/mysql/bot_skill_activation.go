...
const mysqlBotSkillActivationFactColumns = "activation_definition_revision, activation_skill_set_hash, activation_runtime_body_id, activation_installation_id"

type mysqlBotSkillActivationFact struct {
...
func scanMySQLBotSkillActivationFact(row *sql.Row) (mysqlBotSkillActivationFact, error) {
	var fact mysqlBotSkillActivationFact
	err := row.Scan(&fact.definitionRevision, &fact.skillSetHash, &fact.runtimeBodyID, &fact.installationID)
	return fact, err
}

func (fact mysqlBotSkillActivationFact) matches(input types.BotSkillMutationActivationInput) bool {
...
func (a *Adapter) ActivateBotSkillMutation(
	...
	mutation, _, err := scanMySQLBotSkillMutation(tx.QueryRow(
		"SELECT "+mysqlBotSkillMutationColumns+" FROM bot_skill_mutations WHERE bot_uid = ? AND id = ? FOR UPDATE",
	...
	if mutation.Status == types.BotSkillMutationActive {
		fact, factErr := scanMySQLBotSkillActivationFact(tx.QueryRow(
			"SELECT "+mysqlBotSkillActivationFactColumns+" FROM bot_skill_mutations WHERE bot_uid = ? AND id = ?",
			input.BotUID, input.MutationID,
		))
		if factErr != nil {
	...
	if _, err := tx.Exec("UPDATE bot_config SET config = ? WHERE user_id = ?", next, input.BotUID); err != nil {
		return nil, nil, false, fmt.Errorf("save bot definition for skill activation: %w", err)
	}
	result, err := tx.Exec(
		"UPDATE bot_skill_mutations SET status = ?, activated_at = ?, activation_definition_revision = ?, "+
			"activation_skill_set_hash = ?, activation_runtime_body_id = ?, activation_installation_id = ?, "+
			"error_code = NULL, error_summary = NULL WHERE bot_uid = ? AND id = ? AND status = ?",
	...
	if err != nil {
		return nil, nil, false, fmt.Errorf("read bot skill activation result: %w", err)
	}
	if affected != 1 {
		return nil, nil, false, store.ErrBotSkillMutationStateConflict
	}
	...
	if err != nil {
		return nil, nil, false, fmt.Errorf("read activated bot skill mutation: %w", err)
	}
	if err := tx.Commit(); err != nil {
...
func (a *Adapter) RecordBotSkillMutationActivationFailure(
	...
	result, err := tx.Exec(
		"UPDATE bot_skill_mutations SET status = ?, error_code = ?, error_summary = ?, "+
			"activation_definition_revision = ?, activation_skill_set_hash = NULL, "+
	...
	affected, err := result.RowsAffected()
	if err != nil {
		return nil, nil, false, fmt.Errorf("read bot skill activation failure result: %w", err)
	}
	if affected != 1 {
		return nil, nil, false, store.ErrBotSkillMutationStateConflict
	}
	...
	if err != nil {
		return nil, nil, false, fmt.Errorf("read failed bot skill mutation: %w", err)
	}
	if err := tx.Commit(); err != nil {
