...
func TestMySQLArtifactRuntimeStateAndEventShareTransaction(t *testing.T) {
...
func TestMySQLArtifactRuntimeConflictDoesNotAppendEvent(t *testing.T) {
	sqlDB, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
	...
	mock.ExpectBegin()
	mock.ExpectExec(`UPDATE artifact_runtime_states`).WillReturnResult(sqlmock.NewResult(0, 0))
...
func TestMySQLArtifactRuntimeListReadsReferencesOnly(t *testing.T) {
	sqlDB, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("create mock database: %v", err)
	}
