...
func (a *Adapter) SearchAdminUsers(query, field string, limit, offset int) ([]*types.User, int, error) {
	...
	exact := "CAST(id AS CHAR) = ?"
	args := []interface{}{pattern}
	exactArgs := []interface{}{query}
	if field == "name" {
		where = "(LOWER(username) LIKE LOWER(?) ESCAPE '!' OR LOWER(COALESCE(email, '')) LIKE LOWER(?) ESCAPE '!' OR LOWER(display_name) LIKE LOWER(?) ESCAPE '!')"
		exact = "LOWER(username) = LOWER(?) OR LOWER(COALESCE(email, '')) = LOWER(?) OR LOWER(display_name) = LOWER(?)"
		args = []interface{}{pattern, pattern, pattern}
		exactArgs = []interface{}{query, query, query}
	}
	var total int
