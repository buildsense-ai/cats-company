...
func (a *Adapter) CreateSchema() error {
	...
	if err := a.ensureConversationTaskStatusSyncTriggers(); err != nil {
		return err
	}
	for _, statement := range []string{
		reconcileConversationTaskStatusSources,
		backfillConversationTaskStatusSources,
	} {
		if _, err := a.db.Exec(statement); err != nil {
			return fmt.Errorf("reconcile conversation task status sources: %w", err)
		}
	}
	return nil
}

func (a *Adapter) ensureConversationTaskStatusSyncTriggers() error {
	triggers := []struct {
		name      string
		statement string
	}{
		{name: "trg_conversation_task_statuses_sync_source_insert", statement: createConversationTaskStatusSyncInsertTrigger},
		{name: "trg_conversation_task_statuses_sync_source_update", statement: createConversationTaskStatusSyncUpdateTrigger},
	}
	for _, trigger := range triggers {
		...
		if err := a.db.QueryRow(
			...
			return fmt.Errorf("inspect conversation task status trigger %s: %w", trigger.name, err)
		}
		if count != 0 {
			continue
		}
		if _, err := a.db.Exec(trigger.statement); err != nil {
			return fmt.Errorf("create conversation task status trigger %s: %w", trigger.name, err)
		}
	}
	return nil
}
...
const reconcileConversationTaskStatusSources = `
UPDATE conversation_task_status_sources AS source
JOIN conversation_task_statuses AS aggregate
  ON aggregate.topic_id = source.topic_id
 AND aggregate.source_uid = source.source_uid
SET source.run_id = aggregate.run_id,
    source.state = aggregate.state,
    source.summary = aggregate.summary,
    source.error = aggregate.error,
    source.expires_at = aggregate.expires_at,
    source.updated_at = aggregate.updated_at
WHERE aggregate.source_uid IS NOT NULL
  AND NOT (
...
const backfillConversationTaskStatusSources = `
...
const createConversationTaskStatusSyncInsertTrigger = `
CREATE TRIGGER trg_conversation_task_statuses_sync_source_insert
AFTER INSERT ON conversation_task_statuses
FOR EACH ROW
BEGIN
  IF NEW.source_uid IS NOT NULL THEN
    INSERT INTO conversation_task_status_sources
      (topic_id, source_uid, run_id, state, summary, error, expires_at, updated_at)
    SELECT NEW.topic_id, NEW.source_uid, NEW.run_id, NEW.state, NEW.summary, NEW.error, NEW.expires_at, NEW.updated_at
    FROM DUAL
    WHERE NOT EXISTS (
      SELECT 1
      FROM conversation_task_status_sources AS source
      WHERE source.topic_id = NEW.topic_id
        AND source.source_uid = NEW.source_uid
        AND source.state IN ('running', 'waiting')
        AND (source.expires_at IS NULL OR source.expires_at > CURRENT_TIMESTAMP)
        AND source.run_id <> NEW.run_id
        AND NEW.state NOT IN ('running', 'waiting')
    )
...
const createConversationTaskStatusSyncUpdateTrigger = `
CREATE TRIGGER trg_conversation_task_statuses_sync_source_update
AFTER UPDATE ON conversation_task_statuses
FOR EACH ROW
BEGIN
  IF NEW.source_uid IS NOT NULL THEN
    INSERT INTO conversation_task_status_sources
