...
func (s *RedisRuntimeState) bindRuntimeRoute(route runtimeRoute) {
...
func (s *RedisRuntimeState) deliverDeviceRPC(route runtimeRoute, msg *MsgDeviceRPC) bool {
	if s == nil || s.client == nil || msg == nil || route.NodeID == "" || route.ConnectionID == "" {
		return false
	}
	if !s.routeConnected(route) {
		return false
	}
...
func (s *RedisRuntimeState) routeConnected(route runtimeRoute) bool {
	if s == nil || s.client == nil || !route.validAt(time.Now()) {
		return false
	}
...
func (s *RedisRuntimeState) bindUserDeviceRoute(ownerUID int64, device UserDevice, route runtimeRoute) {
	if s == nil || s.client == nil || ownerUID <= 0 || device.DeviceID == "" || route.NodeID == "" || route.ConnectionID == "" {
		return
	}
	s.bindRuntimeRoute(route)
	payload, err := json.Marshal(redisRouteFromRuntime(route))
	if err != nil {
		return
	}
	ttl := ttlUntil(time.Now(), route.ExpiresAt)
	if ttl <= 0 {
		return
	}
