...
func (s *agentIdentityE2EStore) IsUserBot(userID int64) (bool, error) {
...
func (s *agentIdentityE2EStore) GetAccessibleAgent(agentUID, userUID int64) (*types.AgentRosterItem, error) {
	user, ok := s.users[agentUID]
	if !ok || user.AccountType != types.AccountBot {
		return nil, nil
	}
	source := ""
	if owner, ok := s.owners[agentUID]; ok && owner == userUID {
		source = "owner"
	} else if s.friendPairs[agentPairKey(userUID, agentUID)] {
		source = "friend"
	}
	if source == "" {
		return nil, nil
	}
	return &types.AgentRosterItem{
		ID:          agentUID,
		UID:         agentUID,
		...
		CanChat:     true,
		IsBot:       true,
		TopicID:     p2pTopicID(userUID, agentUID),
	}, nil
}
