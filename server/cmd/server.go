...
func main() {
	...
	shimoConnectorHandler := server.NewShimoConnectorHandlerFromEnv()
	shimoConnectorHandler.SetLoginResumePublisher(hub.DeliverShimoLoginResume)
	...
	mux.HandleFunc("/api/artifact-runtime", jwtAuthWithDB(artifactRuntimeHandler.HandleUser))
	...
	mux.HandleFunc("GET /api/agents/{uid}/knowledge/manifest", jwtAuthWithDB(agentHandler.HandleKnowledgeWikiManifest))
	...
	mux.HandleFunc("/api/bot/artifact-runtime", botAPIKeyAuthWithDB(artifactRuntimeHandler.HandleBot))
	...
	mux.HandleFunc("/v1/shimo/connection", shimoConnectorHandler.HandleConnection)
	mux.HandleFunc("/v1/shimo/connection-link", shimoConnectorHandler.HandleConnectionLink)
	mux.HandleFunc("/v1/shimo/sheets/list", shimoConnectorHandler.HandleListSheets)
	mux.HandleFunc("/v1/shimo/sheets/read", shimoConnectorHandler.HandleReadSheet)
	mux.HandleFunc("/v1/shimo/documents/read", shimoConnectorHandler.HandleReadDocument)
	mux.HandleFunc("/connect/shimo/", shimoConnectorHandler.HandleLoginAttempt)
	mux.HandleFunc("/internal/shimo/login-complete", shimoConnectorHandler.HandleLoginComplete)
