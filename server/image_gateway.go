...
func newImageGenerationProxyHandlerWithProviders(providers []imageUpstreamProvider, opts ImageGenerationProxyOptions) *ImageGenerationProxyHandler {
	...
	if handler.raceDeadline <= 0 {
		handler.raceDeadline = defaultImageGenerationTimeout
	}
	if handler.retryBackoff <= 0 {
		handler.retryBackoff = defaultImageRaceBackoff
	}
	...
	if generationProviders == 0 {
		handler.configError = errors.New("image provider pool has no generation provider")
	}
	if editProviders == 0 {
		handler.editConfigError = errors.New("image provider pool has no edit provider")
	}
...
func (h *ImageGenerationProxyHandler) forwardImageRequest(
	...
	defer cancel()
	execution := h.runImageRace(ctx, payload, operation, func(round int, attempt imageAttemptResult) {
		log.Printf("[image-race] attempt operation=%s race_id=%s round=%d provider=%s uid=%d category=%s reason=%s status=%d duration_ms=%d", operation, raceID, round, attempt.providerID, requesterUID, attempt.category, attempt.reason, attempt.status, attempt.duration.Milliseconds())
	})
	if execution.outcome == imageRaceCancelled {
		log.Printf("[image-race] cancelled operation=%s race_id=%s rounds=%d uid=%d duration_ms=%d", operation, raceID, execution.rounds, requesterUID, time.Since(startedAt).Milliseconds())
		return
	}
	if execution.outcome != imageRaceCompleted {
		...
		log.Printf("[image-race] failed operation=%s race_id=%s outcome=%s rounds=%d uid=%d reference_count=%d reference_bytes=%d status=%d duration_ms=%d", operation, raceID, execution.outcome, execution.rounds, requesterUID, referenceCount, referenceBytes, status, time.Since(startedAt).Milliseconds())
		w.Header().Set("X-CatsCo-Image-Race-Id", raceID)
		w.Header().Set("X-CatsCo-Image-Rounds", fmt.Sprintf("%d", execution.rounds))
		writeJSON(w, status, map[string]interface{}{
			"error": map[string]string{
				"code":    code,
				"message": message,
				"race_id": raceID,
				"rounds":  fmt.Sprintf("%d", execution.rounds),
			},
		})
		return
	}
	...
	w.Header().Set("X-CatsCo-Image-Round", fmt.Sprintf("%d", execution.winnerRound))
	w.WriteHeader(winner.status)
	_, _ = w.Write(winner.body)
	log.Printf("[image-race] completed operation=%s race_id=%s round=%d provider=%s uid=%d reference_count=%d reference_bytes=%d status=%d duration_ms=%d", operation, raceID, execution.winnerRound, winner.providerID, requesterUID, referenceCount, referenceBytes, winner.status, time.Since(startedAt).Milliseconds())
}
