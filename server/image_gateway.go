...
const (
	...
	defaultImageRaceDeadline                     = 90 * time.Second
	...
	defaultImageCircuitFailureThreshold          = 3
	maximumImageCircuitFailureThreshold          = 100
	defaultImageCircuitCooldown                  = 5 * time.Minute
	maximumImageCircuitCooldown                  = 24 * time.Hour
...
func NewImageGenerationProxyHandler(upstreamURL string, opts ImageGenerationProxyOptions) *ImageGenerationProxyHandler {
	...
	handler.circuitFailureThreshold = opts.CircuitFailureThreshold
	if handler.circuitFailureThreshold <= 0 {
		handler.circuitFailureThreshold = defaultImageCircuitFailureThreshold
	}
	handler.circuitCooldown = opts.CircuitCooldown
	if handler.circuitCooldown <= 0 {
		handler.circuitCooldown = defaultImageCircuitCooldown
	}
...
func NewImageGenerationProxyHandlerFromEnv() *ImageGenerationProxyHandler {
	...
	circuitFailureThreshold, err := parsePositiveInt64Env(
		"CATSCO_IMAGE_CIRCUIT_FAILURE_THRESHOLD",
		defaultImageCircuitFailureThreshold,
	)
	if err != nil {
		return &ImageGenerationProxyHandler{configError: err}
	}
	if circuitFailureThreshold > maximumImageCircuitFailureThreshold {
		return &ImageGenerationProxyHandler{configError: fmt.Errorf(
			"CATSCO_IMAGE_CIRCUIT_FAILURE_THRESHOLD must not exceed %d",
			maximumImageCircuitFailureThreshold,
		)}
	}
	circuitCooldownSeconds, err := parsePositiveInt64Env(
		"CATSCO_IMAGE_CIRCUIT_COOLDOWN_SECONDS",
		int64(defaultImageCircuitCooldown/time.Second),
	)
	if err != nil {
		return &ImageGenerationProxyHandler{configError: err}
	}
	if time.Duration(circuitCooldownSeconds)*time.Second > maximumImageCircuitCooldown {
		return &ImageGenerationProxyHandler{configError: fmt.Errorf(
			"CATSCO_IMAGE_CIRCUIT_COOLDOWN_SECONDS must not exceed %d seconds",
			int(maximumImageCircuitCooldown/time.Second),
		)}
	}
	...
	if poolFile != "" {
		...
		handler := newImageGenerationProxyHandlerWithProviders(providers, ImageGenerationProxyOptions{
			...
			CircuitFailureThreshold: int(circuitFailureThreshold),
			CircuitCooldown:         time.Duration(circuitCooldownSeconds) * time.Second,
