...
按同一技能可见范围返回服务器 Agent 最近一次实际上报的运行时 Skills 清单。需要用户 JWT；响应只包含脱敏元数据（名称、描述、相对路径和调用标记），不包含哈希、服务器绝对路径、`SKILL.md` 内容或凭据。`runtime_status` 为 `unreported`、`reported` 或 `stale`；超过 15 分钟未收到服务端回执时为 `stale`。
