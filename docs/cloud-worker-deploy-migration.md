...
- 网页后台转发同样使用专用预算：部署脚本 600 秒 → Relay 630 秒 → CatsCompany 管理代理 650 秒 → Nginx 660 秒。其他管理请求保留原 60 秒预算。既有生产 Nginx 更新脚本会同时补齐云员工入口与后台自动部署入口，覆盖已有域名配置。
- Nginx 有宿主机和 Web 容器两跳，二者都要配置后台自动部署专用路由；只调整宿主机会被容器默认的 60 秒再次截断。新包月实例的自动续费配置若返回 `Ecs.Instance.UpdateError`，最多尝试 5 次、间隔 5 秒；其他错误立即失败，耗尽后仍走退订清理，不会放行未知续费状态或重试购买实例。
