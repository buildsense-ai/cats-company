...
| `catsco-personal` | Personal / 个人版 | ¥399 | 2 亿 | ¥11000 | 30 天（月套餐） | 不限 | `test` |
| `catsco-pro` | Pro / 专业版 | ¥799 | 6 亿 | ¥33000 | 30 天（月套餐） | 不限 | `test`，用户侧标记推荐 |
...
Personal 和 Pro 每次成功支付各赠送 1 次云托管员工创建权益，并在支付完成后**自动创建**云托管员工（支付宝回调与对账补单两条路径共用同一钩子，每笔订单只触发一次）。自动创建复用与手动创建完全相同的权益预留、失败回滚与自动加好友流程；账号已有云托管员工时（续费、升级或重复购买）不重复创建，仅执行实例续订与生命周期顺延。自动创建期间遇到并发云操作（冲突）会做有界重试，最终失败只写服务器日志（`CRITICAL` 标记）并保留权益供用户手动创建。创建失败会释放该次权益；创建成功后即消耗。当前套餐可通过“续费”按钮再次购买：续费周期从当前套餐到期时间顺延 30 天，早续不会损失剩余天数；升级仍按立即切换规则处理。平台不签约支付宝自动扣款，所有续费均由用户手动发起普通支付宝支付；支付成功后，控制面才调用天翼云 `ResubscribeEcsInstance` 续订或恢复实例。套餐到期时 CatsCompany 只归档生命周期，不主动退订云主机，让实例自然进入天翼云 `expired/freezing` 冻结保留状态；保留期内续费会恢复实例并再次强制关闭云侧自动续费。`unsubscribed`、`released` 或 `deleted` 实例不可恢复，续费脚本会明确拒绝，也不会自动新建替代实例。Personal → Pro 立即升级时，旧 Personal 尚未使用的创建权益会一并撤销，Pro 只保留新套餐赠送的 1 次；已经进入创建流程的预留权益会继续完成并计为已使用。退款只撤销尚未使用/预留的创建权益和模型额度；已经创建的云员工不因退款立即销毁，继续运行至当次套餐到期，再按正常 15 天保留期和生命周期清理规则处理。用户侧不提供云托管员工永久删除能力；删除会话或从任务列表移除只影响任务入口，不影响云员工。云员工使用期限跟随当次套餐 30 天有效期；到期后天翼云进入 15 天冻结保留期，实例不可用且不收费，用户应在保留期内续费以触发恢复；超过窗口后由内部生命周期任务执行退订和永久销毁，随后清理 key pair、本地状态、机器人记录与生命周期记录。订单、支付与权益审计记录永久保留。
...
当前新套餐开放五个公开聊天模型（MiniMax M2.7、MiniMax M3、DeepSeek V4 Flash、GLM-5.3 Flash、GPT-5.6 Terra）与五个生图模型（gpt-image-2、gpt-image-2.5、gpt-image-2.5-flare、gpt-image-2.5-sunburst、chatgpt-image-latest）。所有模型共用同一个模型服务额度池；套餐表中的分模型数值只用于声明可用模型并组成共享总额，不是各模型独立额度：Personal 在五个聊天模型下各记 ¥2100、五个生图模型下各记 ¥100，汇总为 ¥11000；Pro 各记 ¥6300、各记 ¥300，汇总为 ¥33000。任一模型的调用都按自身价格倍率从同一池中扣减，因此不同模型的 token 或出图张数消耗速度不同，但不会把套餐总额复制多份。生图模型只通过技能接口调用，不进入用户可切换的模型列表；Free 套餐默认不含生图模型，确需为个别账户开通时由内部运营在商业化后台显式补额（必须填写到期时间），不会随套餐自动放开。Pro 的内部等价 token 与共享总额均为 Personal 的 3 倍，对应官网“约 3 倍任务容量”的表达。新增公开模型时必须同步更新套餐模型集合、Relay provider scope、价格覆盖和存量权益迁移，不能只改模型目录。
