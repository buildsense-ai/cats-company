...
updates = {
    ...
    "CATSCO_ARTIFACT_DNS_ACCESS_KEY": os.environ.get("CATSCO_ARTIFACT_DNS_ACCESS_KEY", ""),
    "CATSCO_ARTIFACT_DNS_SECRET_KEY": os.environ.get("CATSCO_ARTIFACT_DNS_SECRET_KEY", ""),
}
...
# The web container resolves `server` through the Docker DNS resolver on
# every request (deploy/nginx/nginx.conf), so a server recreation no longer
# strands nginx on the previous address and no follow-up web recreate is
# needed.  Waiting for web health below still guards deploys that recreate
...
if printf '%s' "$compose_profiles" | tr ',' '\n' | grep -qx 'shimo'; then
  shimo_worker_port="$(sed -n 's/^PROD_SHIMO_WORKER_HOST_PORT=//p' "$env_file" | tail -n 1)"
