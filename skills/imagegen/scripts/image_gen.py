#!/usr/bin/env python3
"""Codex-compatible CLI for image generation and editing with GPT Image models.

This is the CLI execution backend for the CatsCo ``imagegen`` skill.  Its
command and prompt-augmentation surface follows the Codex imagegen CLI, while
authentication can use either an OpenAI key or the current CatsCo bot/user
identity.  Secrets are read only from the process environment.
"""
...
def _catsco_base_url() -> Optional[str]:
    explicit = os.getenv("CATSCO_IMAGE_API_BASE", "").strip()
    if explicit:
        return explicit.rstrip("/")
    service = (
        os.getenv("CATSCO_HTTP_BASE_URL", "").strip()
        or os.getenv("CATSCOMPANY_HTTP_BASE_URL", "").strip()
    )
    if not service:
        return None
    return service.rstrip("/") + "/v1"


def _configured_identity() -> Optional[Tuple[str, str]]:
    bot_key = (
        os.getenv("CATSCO_API_KEY", "").strip()
        or os.getenv("CATSCOMPANY_API_KEY", "").strip()
    )
    if bot_key:
        # The OpenAI SDK always presents api_key as a Bearer token. CatsCo's
        # OpenAI-compatible image routes accept bot keys under the same scheme,
        # so this adapter behaves exactly like a stock OpenAI client.
        return "Bearer", bot_key
    user_token = (
        os.getenv("CATSCO_USER_TOKEN", "").strip()
        or os.getenv("CATSCOMPANY_USER_TOKEN", "").strip()
    )
    if user_token:
        return "Bearer", user_token
    openai_key = os.getenv("OPENAI_API_KEY", "").strip()
    if openai_key:
        return "Bearer", openai_key
    return None


def _ensure_api_key(dry_run: bool) -> None:
    if _configured_identity() is not None:
        print("Image API identity is configured.", file=sys.stderr)
        return
    if dry_run:
        _warn("No Image API identity is configured; dry-run only.")
        return
    _die(
        "No Image API identity is configured. Set CATSCO_API_KEY for the "
        "CatsCo relay, CATSCO_USER_TOKEN for a user session, or OPENAI_API_KEY "
        "for a direct OpenAI call."
    )
...
def _client_options() -> Dict[str, Any]:
    ...
    if identity is None:
        _die("No Image API identity is configured.")
    _, secret = identity
    catsco_base = _catsco_base_url()
    direct_base = os.getenv("IMAGE_GEN_API_BASE", "").strip()
    options: Dict[str, Any] = {"api_key": secret}
    base_url = catsco_base or direct_base
    if base_url:
