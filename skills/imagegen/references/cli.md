# Explicit API-control CLI (`scripts/image_gen.py`)

Read this file when the request needs exact Image API/model controls beyond the default three-field tool-compatible schema.

`generate-batch` is a full-CLI subcommand. The word `batch` alone does not require it when separate tool-compatible calls are sufficient.
...
Real API calls require network access and either a CatsCo runtime identity or `OPENAI_API_KEY`. `--dry-run` does not.

## Quick start (works from any repo)
Set a stable path to the installed Skill directory:

```
export IMAGEGEN_SKILL_DIR="/srv/catsco-agent/skills/imagegen"
...
Generate (requires configured identity + network):
...
- Do not silently switch from `gpt-image-2` to another model.
...
`gpt-image-2` is the default model for explicit API-control work.
...
- Use `--input-fidelity high` for edits that must preserve identity, typography, layout, or product details; use `low` only for looser reinterpretation.
- Current GPT-Image-2 supports `--background transparent` in preview with PNG or WebP output.
...
Transparent GPT-Image-2 request:
...
These are explicit CLI controls. They are not fields in the tool-compatible request schema.
