...
These prompting principles are shared by both CLI-backed modes of the skill:
- tool-compatible `scripts/invoke_imagegen.py` (default)
- explicit API-control `scripts/image_gen.py`

This file is about prompt structure, specificity, and iteration. Explicit API controls such as `quality`, `input_fidelity`, masks, output format, and output paths live in the API-control docs.
...
- [Explicit API execution controls](#explicit-api-execution-controls)
...
- In explicit API-control mode, use `medium` or `high` quality for small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes.
...
## Transparent images
- Ask for a genuinely transparent background and preserve its alpha.

## Explicit API execution controls
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are explicit API-control options.
- They are not fields in the three-field tool-compatible request schema.
- See `references/cli.md` and `references/image-api.md` when those controls are needed.
...
- For identity-, typography-, layout-, or product-sensitive edits, explicit API-control mode may set `input_fidelity=high`.
- Current GPT-Image-2 supports `background=transparent` in preview; use PNG or WebP for alpha output.
