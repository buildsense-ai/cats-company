...
These prompt recipes are shared across both modes of the Skill:
- tool-compatible `scripts/invoke_imagegen.py` (default)
- `scripts/image_gen.py` for explicit API/model controls
...
Execution details such as `quality`, `input_fidelity`, masks, output formats, and local output paths depend on mode. Use the tool-compatible CLI by default, request transparent backgrounds directly, and preserve the generated alpha; use the full CLI when explicit controls are required.
...
- Use `input_fidelity=high` for identity-, typography-, layout-, or product-sensitive edits; use `low` only when looser reinterpretation is desired.
