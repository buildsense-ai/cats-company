...
Read this file when a CLI-backed image call cannot reach the configured CatsCo or direct OpenAI endpoint.

This guidance is intentionally isolated from `SKILL.md` because it can vary by environment and may become stale. Prefer the defaults in your environment when in doubt.

## Why am I asked to approve image generation calls?
The Skill CLI uses an OpenAI-compatible Image API, so it needs outbound access to the configured CatsCo relay or direct OpenAI endpoint.
